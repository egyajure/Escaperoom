#include <iostream>
#include <vector>
#include "Square.cpp" //so this file can use the class created
//(created second)

//goal: to create a function that reads input and stores it in a vector, returns nothing

void gatherData(std::vector<Square> &vect, int x, int y){ //creates a reference to the vector "vect" which holds Squares
    int xCoordinate = 1;
    int yCoordinate = 1;
        
    for (int i = 0; i < x * y; i++){ //looping through each value
